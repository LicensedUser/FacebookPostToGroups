This Python script automates the process of sharing a Facebook photo post to multiple Facebook groups using screen automation via pyautogui.
The workflow is divided into batches, each targeting a set number of groups per cycle.
...
The script includes retry logic for image recognition, logging for process tracking, and a counter to maintain the correct group targeting across iterations.
    ...
    ⚠️ Note: This script relies on image-based UI automation and may be affected by screen resolution, browser zoom, or UI changes on Facebook. 
    It is your responsibility to ensure compliance with Facebook's terms of service and to use automation ethically.
